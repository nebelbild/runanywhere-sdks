...
rac_result_t rac_model_check_compatibility(rac_model_registry_handle_t registry_handle,
    ...
    if (result != RAC_SUCCESS) {
        RAC_LOG_WARNING("ModelCompatibility", "Failed to get model from registry: %s (error: %d)",
                        model_id, result);
        return result;
    }
    ...
    RAC_LOG_DEBUG("ModelCompatibility",
                  "Model %s requirements: memory=%lld bytes, storage=%lld bytes", model_id,
    ...
    out_result->is_compatible = (can_run == RAC_TRUE && can_fit == RAC_TRUE) ? RAC_TRUE : RAC_FALSE;
    ...
    RAC_LOG_INFO("ModelCompatibility",
                 ...
                 model_id, can_run, can_fit, out_result->is_compatible,
                 static_cast<long long>(available_ram), static_cast<long long>(required_memory),
